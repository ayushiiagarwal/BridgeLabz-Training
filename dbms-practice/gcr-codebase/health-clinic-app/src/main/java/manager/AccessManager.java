...
public class AccessManager {
    public void addSpecialty(Speciality specialty) throws ClinicException, SQLException {
        String sql = """
        ...
        try (Connection connection = DBconnection.getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, specialty.getSpecialityName());
    ...
    public void updateSpecialty(Speciality specialty) throws ClinicException, SQLException {
        String sql = """
            UPDATE specialties
        ...
        try (Connection connection = DBconnection.getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, specialty.getSpecialityName());
            ps.setString(2, specialty.getDescription());
            ps.setInt(3, specialty.getSpecialityId());

            int rows = ps.executeUpdate();
            if (rows == 0) {
                throw new ClinicException("Speciality not found.");
            }
